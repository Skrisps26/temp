"""
train_gpt_shift.py  —  Factorized-Depth Transformer (INT4 Packed + FP16 Adapters)
===================================================================================
Architecture (14 Virtual Layers, d=1024 | 180M Effective Params, 17M Physical):
  - 1 Shared Base Block (Compressed to INT4 Packed)
  - 14 Unique Rank-16 LoRA Adapters (Kept in pristine FP16)
  - Evaluates with the reasoning power of a 180M model but zips to < 14MB.
  - EMA pauses during the 2% Late-QAT phase to prevent moving-average corruption.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# HYPERPARAMETERS (Tuned for 8x H100, Factorized Depth)
# ─────────────────────────────────────────────────────────────────────────────
class Hyperparameters:
    ...
    num_layers    = int(os.environ.get("NUM_LAYERS",    14)) # 14 Virtual Layers
    model_dim     = int(os.environ.get("MODEL_DIM",     1024)) # Massive Shared Width
    ...
    mousse_backend_steps  = int(os.environ.get("MOUSSE_BACKEND_STEPS",    10)) 
    ...
    late_qat_frac         = float(os.environ.get("LATE_QAT_FRAC",         0.02)) # Shrunk to 2% to prevent shock
...
CONTROL_PATTERNS = tuple(p for p in os.environ.get(
    ...
    "attn_scale,mlp_scale,resid_mix,q_gain,depth_gate,shift_alpha",
).split(",") if p)
# LoRA paths are kept in pristine FP16
FP16_PATTERNS  = ("tok_emb.weight", "lora_A", "lora_B")
...
def unpack_int4_tensor(packed: Tensor, original_shape: list, per_row: bool, scale: Tensor) -> Tensor:
    high_bits, low_bits = (packed >> 4) & 0x0F, packed & 0x0F
    unpacked = torch.empty(packed.numel() * 2, dtype=torch.uint8, device=packed.device)
    unpacked[0::2], unpacked[1::2] = high_bits, low_bits
    
    unpacked_int8 = unpacked.to(torch.int8) - 8
    original_numel = int(np.prod(original_shape))
    if unpacked_int8.numel() > original_numel: unpacked_int8 = unpacked_int8[:original_numel]
...
def eval_val(args, model, rank, world_size, device, gas, val_tokens, bb, hs, ib):
    ...
    with torch.inference_mode():
        for bss in range(ss, se, lbs):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True): bl = model(x, y).detach()
            n = float(y.numel())
...
def eval_val_sliding_window(args, base_model, rank, world_size, device, val_tokens, bb, hs, ib):
    ...
    with torch.inference_mode():
        for bi in range(0, len(my_starts), WB):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True): logits = base_model.forward_logits(xs)
...
# ─────────────────────────────────────────────────────────────────────────────
# FACTORIZED DEPTH ARCHITECTURE BLOCKS
# ─────────────────────────────────────────────────────────────────────────────
...
class SharedBaseBlock(nn.Module):
    """The massive, 4-bit compressed heavy-lifter. Only ONE instantiated."""
    def __init__(self, dim, num_heads, num_kv_heads, mlp_mult):
        super().__init__()
        hd = dim // num_heads
        qkv_dim = (num_heads + 2 * num_kv_heads) * hd
        hidden = dim * mlp_mult

        self.attn_qkv = CastedLinear(dim, qkv_dim, bias=False)
        self.attn_out = CastedLinear(num_heads * hd, dim, bias=False)
        self.attn_out._zero_init = True

        self.mlp_gate_up = CastedLinear(dim, hidden * 2, bias=False)
        self.mlp_down    = CastedLinear(hidden, dim, bias=False)
        self.mlp_down._zero_init = True
        
        for w in (self.attn_qkv, self.mlp_gate_up): _ortho(w.weight, dim ** -0.5)

class LayerAdapter(nn.Module):
    """The tiny, unshared steering mechanism for each specific depth (14 instantiated)."""
    def __init__(self, dim, num_heads, num_kv_heads, mlp_mult, rank, layer_idx):
        super().__init__()
        ...
        # Microscopic LoRA paths (Rank 16)
        self.qkv_lora_A = CastedLinear(dim, rank, bias=False)
        self.qkv_lora_B = CastedLinear(rank, qkv_dim, bias=False)
        self.qkv_lora_B._zero_init = True

        self.out_lora_A = CastedLinear(num_heads * hd, rank, bias=False)
        self.out_lora_B = CastedLinear(rank, dim, bias=False)
        self.out_lora_B._zero_init = True

        self.mlp_lora_A = CastedLinear(dim, rank, bias=False)
        self.mlp_lora_B = CastedLinear(rank, hidden * 2, bias=False)
        self.mlp_lora_B._zero_init = True

        self.down_lora_A = CastedLinear(hidden, rank, bias=False)
        self.down_lora_B = CastedLinear(rank, dim, bias=False)
        self.down_lora_B._zero_init = True

        # Learnable layer scales to differentiate the depths
...
class GPT(nn.Module):
    def __init__(self, args: Hyperparameters):
        ...
        self.rotary  = ProgressiveRotary(args.model_dim // args.num_heads, base=args.rope_base)
        self._xsa_mask_cache = {}

        # The core of Factorized Depth
        self.shared_base = SharedBaseBlock(args.model_dim, args.num_heads, args.num_kv_heads, args.mlp_mult)
        self.adapters = nn.ModuleList([
            LayerAdapter(args.model_dim, args.num_heads, args.num_kv_heads, args.mlp_mult, args.adapter_rank, i) 
    ...
    def _forward_attn(self, x, adapter, base, cos, sin, rope_dims, use_xsa):
        B, T, _ = x.shape
        # Heavy Shared Math + Tiny Adapter Steering
        qkv = base.attn_qkv(x) + adapter.qkv_lora_B(adapter.qkv_lora_A(x))
        ...
        q = apply_rope_partial(q, cos, sin, rope_dims) * adapter.q_gain.to(q.dtype)[None, :, None, None]
        ...
        if use_xsa and HAS_FA3:
            y = F.scaled_dot_product_attention(q, k, v, attn_mask=self._get_xsa_mask(T, x.device, q.dtype), is_causal=False)
        elif HAS_FA3:
        ...
        y = y.transpose(1, 2).contiguous().reshape(B, T, nh * hd)
        return base.attn_out(y) + adapter.out_lora_B(adapter.out_lora_A(y))

    def _forward_mlp(self, x, adapter, base):
        gu = base.mlp_gate_up(x) + adapter.mlp_lora_B(adapter.mlp_lora_A(x))
        hidden = gu.shape[-1] // 2
        act = torch.relu(gu[..., :hidden] * self.args.mlp_temperature).square() * gu[..., hidden:]
        return base.mlp_down(act) + adapter.down_lora_B(adapter.down_lora_A(act))

    def _backbone(self, ids: Tensor) -> Tensor:
        x = F.rms_norm(self.tok_emb(ids), (self.args.model_dim,))
        x0 = x
        cos, sin = self.rotary(ids.shape[1], x.device, x.dtype)

        # Unroll 14 virtual layers through the shared block
        xsa_start = self.args.num_layers - self.args.xsa_layers
        for i in range(self.args.num_layers):
            adapter = self.adapters[i]
            
            mix = adapter.resid_mix.to(x.dtype)
            current_x = mix[0][None, None] * x + mix[1][None, None] * x0
            
            attn_out = self._forward_attn(adapter.attn_norm(current_x), adapter, self.shared_base, cos, sin, self.rope_dims_list[i], (i >= xsa_start))
            x = current_x + adapter.depth_gate.to(x.dtype) * adapter.attn_scale.to(x.dtype)[None, None] * attn_out * adapter.ln_scale
            
            mlp_out = self._forward_mlp(adapter.mlp_norm(x), adapter, self.shared_base)
            x = x + adapter.depth_gate.to(x.dtype) * adapter.mlp_scale.to(x.dtype)[None, None] * mlp_out * adapter.ln_scale
    ...
    def apply_late_qat(self):
        """Quantizes ONLY the Shared Base matrices to 4-bit. Adapters stay FP16/FP32."""
        with torch.no_grad():
            for name, p in self.named_parameters():
                if "shared_base" in name and p.ndim == 2:
...
def main():
    ...
    matrix_p = [p for n, p in base_model.named_parameters() if p.ndim == 2 and not any(pat in n for pat in CONTROL_PATTERNS) and "tok_emb" not in n]
    scalar_p = [p for n, p in base_model.named_parameters() if p.ndim < 2 or any(pat in n for pat in CONTROL_PATTERNS)]
