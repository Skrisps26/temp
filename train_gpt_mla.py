...
class Hyperparameters:
    ...
    # head_dim = 512/8 = 64
    rope_dims_early = int(os.environ.get("ROPE_DIMS_EARLY", 8))   # layers 0..num_layers//3
...
class Mousse(torch.optim.Optimizer):
    ...
    def __init__(self, params, lr, momentum, backend_steps,
                 beta2=0.999, eps=1e-8, nesterov=True, wd=0.0):
        super().__init__(params, dict(lr=lr, momentum=momentum,
                                      backend_steps=backend_steps, beta2=beta2,
                                      eps=eps, nesterov=nesterov, wd=wd))
...
def _quantize_tensor_int6(t: Tensor):
    ...
    if t32.ndim == 2:
        ...
        q     = torch.clamp(torch.round(
                ...
                ), INT6_LO, INT6_HI).to(torch.int8).contiguous()
    ...
    q     = torch.clamp(torch.round(
                torch.clamp(t32, -clip, clip) / scale
            ), INT6_LO, INT6_HI).to(torch.int8).contiguous()
    return q, scale
...
def quantize_state_dict(sd: dict) -> tuple[bytes, dict]:
    ...
    for name, tensor in sd.items():
        ...
        if any(p in name for p in FP16_PATTERNS):
            ...
            meta[name] = {"kind": "fp16", "shape": list(t.shape),
                          "offset": buf.tell(), "n": len(data)}
            buf.write(data)
        elif (not t.is_floating_point() or t.numel() <= MAX_FLOAT_NUMEL
              or any(p in name for p in CONTROL_PATTERNS)):
            data = t.to(torch.float32).numpy().tobytes()
            meta[name] = {"kind": "fp32", "shape": list(t.shape),
            ...
            meta[name] = {"kind": "int6", "shape": list(t.shape),
                          "dtype": str(t.dtype).removeprefix("torch."),
                          "per_row": (t.ndim == 2),
    ...
    for name, m in meta.items():
        ...
        if kind == "fp16":
            arr = np.frombuffer(raw, dtype=np.float16,
                                count=int(np.prod(m["shape"])), offset=m["offset"])
            ...
            q     = torch.from_numpy(np.frombuffer(raw, dtype=np.int8,
                    count=int(np.prod(m["shape"])), offset=m["q_off"]).copy()
                    ).reshape(m["shape"]).float()
            scale = torch.from_numpy(np.frombuffer(raw, dtype=np.float16,
                    count=int(np.prod(m["scale_shape"])), offset=m["s_off"]).copy()
                    ).reshape(m["scale_shape"]).float()
            dq    = (q * scale.view(-1, *([1] * (q.ndim - 1)))
                     if m["per_row"] else q * float(scale.item()))
...
def eval_val(args, model, rank, world_size, device, gas, val_tokens, bb, hs, ib):
    ...
    with torch.inference_mode():
        for bss in range(ss, se, lbs):
            bse  = min(bss + lbs, se)
            local = val_tokens[bss*args.train_seq_len : bse*args.train_seq_len+1].to(
                device=device, dtype=torch.int64)
            x, y = local[:-1].reshape(-1, args.train_seq_len), \
...
@torch.no_grad()
def eval_val_sliding_window(args, base_model, rank, world_size, device,
                             val_tokens, bb, hs, ib):
    ...
    with torch.inference_mode():
        for bi in range(0, len(my_starts), WB):
            ...
            ptl = F.cross_entropy(logits.float().reshape(-1, args.vocab_size),
...
class CausalSelfAttention(nn.Module):
    def __init__(self, dim, num_heads, num_kv_heads, rope_dims, use_xsa,
        ...
        for w in (self.c_q, self.c_k, self.c_v):
            _ortho(w.weight, s)
        # Cached XSA mask — built once per (T, device, dtype) combination
        self._xsa_mask_cache: dict = {}

    def _get_xsa_mask(self, T: int, device, dtype) -> Tensor:
        """
        XSA mask: causal (upper triangle = -inf) + diagonal blocked (-inf).
        Cached per (T, device, dtype) — only built once per sequence length.
        FA2/FA3 cannot accept custom masks, so XSA layers use SDPA.
        Non-XSA layers use FA2/FA3 for full speed.
        """
        ...
        if key not in self._xsa_mask_cache:
            ...
            # Block diagonal: position i cannot attend to itself
            mask.diagonal().fill_(float("-inf"))
            self._xsa_mask_cache = {key: mask}  # keep only latest to save memory
...
class GPT(nn.Module):
    def __init__(self, args: Hyperparameters):
        ...
        self.tok_emb = ShiftMixedEmbedding(args.vocab_size, args.model_dim,
                                           args.shift_alpha_init)

        self.num_enc  = args.num_layers // 2
        self.num_dec  = args.num_layers - self.num_enc
        self.num_skips= min(self.num_enc, self.num_dec)
        self.skip_weights = nn.Parameter(torch.ones(self.num_skips, args.model_dim,
                                                    dtype=torch.float32))
        ...
        self.lm_head    = None if args.tie_embeddings else \
                          CastedLinear(args.model_dim, args.vocab_size, bias=False)
    ...
    def forward_logits(self, ids: Tensor) -> Tensor:
        ...
        lp = (F.linear(x, self.tok_emb.weight.weight)
    ...
    def apply_late_qat(self):
        ...
        with torch.no_grad():
            for name, p in self.named_parameters():
                if (p.ndim == 2 and p.numel() > MAX_FLOAT_NUMEL
...
def main():
    ...
    if not HAS_FA3:
        ...
        enable_cudnn_sdp(False); enable_flash_sdp(flash)
        enable_mem_efficient_sdp(False); enable_math_sdp(not flash)
    ...
    if not use_compile: log("[info] torch.compile disabled")
    ...
    matrix_p = [p for n, p in block_np
                 if p.ndim == 2 and not any(pat in n for pat in CONTROL_PATTERNS)]
    scalar_p  = [p for n, p in block_np
    ...
    opt_mousse = Mousse(matrix_p, lr=args.matrix_lr, momentum=args.mousse_momentum,
                        backend_steps=args.mousse_backend_steps,
                        beta2=args.mousse_beta2, wd=args.mousse_wd)
    ...
    rope_str = (f"early={args.rope_dims_early} mid={args.rope_dims_mid} "
                f"late={args.rope_dims_late}")
    log(f"params:{nparams}  layers:{args.num_layers}  dim:{args.model_dim}  "
    ...
    log(f"batch:{args.train_batch_tokens}  seq:{args.train_seq_len}  "
    ...
    mwms = 1000.0 * args.max_wallclock_seconds if args.max_wallclock_seconds > 0 else None
    ...
    if args.warmup_steps > 0:
        ...
        for ws_i in range(args.warmup_steps):
            ...
            zero_grad()
            if args.warmup_steps <= 20 or (ws_i+1) % 10 == 0 or ws_i+1 == args.warmup_steps:
    ...
    while True:
        ...
        if not last and args.val_loss_every > 0 and step % args.val_loss_every == 0:
            ...
            vl, vb = eval_val(args, model, rank, world_size, device, gas,
                              val_tokens, bb, hs, ib)
        ...
        if args.train_log_every > 0 and (step <= 10 or step % args.train_log_every == 0
            ...
            log(f"step:{step}/{args.iterations} train_loss:{tl.item():.4f} "
    ...
    log(f"pre_quant val_loss:{swl:.4f} val_bpb:{swb:.4f} "
        f"eval_time:{1000.*(time.perf_counter()-tsw):.0f}ms")
    ...
    log(f"roundtrip val_loss:{rtl:.4f} val_bpb:{rtb:.4f} "
        f"eval_time:{1000.*(time.perf_counter()-trt):.0f}ms")
